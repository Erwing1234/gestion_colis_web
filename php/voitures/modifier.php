...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE voiture SET design = '$design', codeit = '$codeit' WHERE idvoit = '$id'";
    
    if ($conn->query($sql) === TRUE) {
        $success = "Voiture mise à jour avec succès.";
        // Mettre à jour les informations affichées
        $voiture['design'] = $design;
        $voiture['codeit'] = $codeit;
    } else {
...
<head>
    ...
    <title>Modifier une Voiture - Gestion des colis</title>
        ...
        <div class="row">
            <div class="col-md-8 offset-md-2">
                <div class="form-container">
                    <h2 class="mb-4"><i class="fas fa-car me-2"></i>Modifier une voiture</h2>
                    ...
                    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . "?id=" . $id); ?>" class="needs-validation" novalidate>
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="idvoit" name="idvoit" value="<?php echo htmlspecialchars($voiture['idvoit']); ?>" readonly>
                            <div class="form-text">L'identifiant ne peut pas être modifié.</div>
                        </div>
                        <div class="mb-3">
                            <label for="design" class="form-label">Désignation</label>
                            <input type="text" class="form-control" id="design" name="design" value="<?php echo htmlspecialchars($voiture['design']); ?>" required>
                            ...
                            <select class="form-select" id="codeit" name="codeit" required>
                                ...
                                if ($result_itineraires->num_rows > 0) {
                                    while($row = $result_itineraires->fetch_assoc()) {
                                        $selected = ($row['codeit'] == $voiture['codeit']) ? 'selected' : '';
                                        echo "<option value='" . $row['codeit'] . "' $selected>" . $row['codeit'] . " - " . $row['villedep'] . " à " . $row['villearr'] . "</option>";
                                    }
                                }
                                ?>
                            </select>
